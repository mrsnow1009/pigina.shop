...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		if ($('#table-branch').length == 1) {
		   	var table_donate = $('#table-branch').DataTable({
		   		order: [[2, 'asc']],
		      	pageLength: <?php echo _MAX_RECORD_BRANCH;?>,
		      	// "lengthChange": false,
		      	scrollCollapse: _scrollCollapse,
		  		scrollX: _scrollX,
		      	columnDefs: [
		        	{ searchable: false, orderable: false, targets: [0,1,5] },
				...
				serverSide: true,
			    ajax: {
			        "url": "<?php echo _ROOT_PATH_ADMIN_AJAX;?>?q=_branch_list",
			        type: 'POST'
			    }
		   	});
	        ...
	        $('#table-branch tbody').on('click', 'td.dt-control', function () {
