...
<script>
	$(document).ready(function(){
		...
		$('#cbx_lang').on('change', function() {
		  	...
		  	$.ajax({
			  	'url': '<?php echo _ROOT_PATH_ADMIN_AJAX;?>?q=html-option-cate&lang='+_lang+'&node=<?php echo $node; ?>&cate=<?php echo $id; ?>',
			  	dataType: 'json'
			}).done(function(data) {
		  		console.log(data);
		  		if (data.error == "true") {
		  			$('#cbx_parentid').html(data.html);
		  		}else{
		  			alert(data.html);
  		 			location.reload(); 
		  		}
		...
		$( "#form_updateCategory" ).validate( {
			rules: {
				cbx_lang: "required",
				cbx_parentid: "required",
				txt_title: "required",
				txt_urlseo: "required",
				cbx_status: "required",
				txt_sort: {
			...
			messages: {
				...
				cbx_parentid: "<?php echo $LANG['this_field_is_required']; ?>",
