...
class banner_controller {
	private $table = TBLBANNER;
	private $module = 'RSBANNER';
	private $_id;
	...
	private function format_std($object = array()){
		...
		isset($object['target']) ? $result->target = $object['target'] : $result->target = '';
		isset($object['script']) ? $result->script = $object['script'] : $result->script = '';
		isset($object['url']) ? $result->url = $object['url'] : $result->url = '';
		isset($object['type']) ? $result->type = $object['type'] : $result->type = '';
		isset($object['expired_date']) ? $result->expired_date = $object['expired_date'] : $result->expired_date = '';
		isset($object['started_date']) ? $result->started_date = $object['started_date'] : $result->started_date = '';
	...
	function DeleteRecord($str_id){
		...
		for($i=0;$i<count($arrId);$i++){
			...
			$rf = $this->Delete(_LEVEL_ADMIN._PATH_UPLOAD_BANNER.$arrId[$i]);/* delete folder */

			/* xoa file slide */
			$_library_controller->delWithNodeId($arrId[$i],$this->module);
		}
