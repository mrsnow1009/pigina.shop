...
class Session
{
	...
	public function __construct ($cook = "zipcook") {
		...
		$this->preStr= "_RS_";		//prefix for cookies
	...
	public function set($var,$value) {
		if ($this->use_cookie=="yes") {
			setcookie($var,$this->crypta($value),time()+$this->duratacookie,"/" );
		} elseif ($this->use_cookie=="zipcook") {
			if ($this->stringone!="") {
			...
			if (count($arr) < $cLiberi) {
				...
				for ($i=0;$i<count($arr);$i++) {
					setcookie($this->preStr.$i,$arr[$i],time()+$this->duratacookie,"/" );
				}
			} else {
				//cookie overflow
	...
	public function finish() {
		if ($this->use_cookie=="yes") {
			...
			for ($x=0;$x<count($cookiesSet);$x++) {
				...
				setcookie($cookiesSet[$x],"",time()-3600*24,"/");	//faccio scadere il cookie
			}
			...
			for ($x=0;$x<count($cookiesSet);$x++) {
				if (strpos(" ".$cookiesSet[$x],$this->preStr)==1){
					setcookie($cookiesSet[$x],"",time()-3600*24,"/");
				}
				$this->stringone="";
			}
// 			return true;
